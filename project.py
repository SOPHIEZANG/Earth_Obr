...
def get_NDVI(path,city):  
    city=city.lower()             
    band_red=rasterio.open(os.path.join(path,city+"NEW_NEW_merge_output_B4.tif"))
    
    red = band_red.read(1).astype('float64')

    band_nir = rasterio.open(os.path.join(path,city + "NEW_NEW_merge_output_B5.tif"))
...
def get_band(path,City,count):
    ...
    with rasterio.open("merge_output"+str(count)+".tif") as src:                       
        ...
        new_filename = os.path.join(path,city2+"NEW_NEW_merge_output_B"+str(count)+".tif")
...
def main():
    ...
    bands=[]
    band=input("which band will you download? ")
    bands=band.split()
